...
- `src/core/` — Transport abstraction (`ITransport` → `SerialTransport`, `TcpTransport`), LP-100A protocol parser (`LP100AProtocol`), Hamlib rig wrapper (`HamlibRig`, direct C API)
- `src/vcp/` — Main VCP window, custom gauge widgets (`PowerGauge`, `SWRGauge`), connection dialog. Three view styles: Compact, Standard, Full (hides/shows impedance and button sections)
...
- **HamlibRig** wraps the Hamlib C API directly (no external rigctld). Suppresses Hamlib debug output via `rig_set_debug_file(/dev/null)` in a static initializer. Pre-warms macOS network stack at app startup for TCP rig connections. Uses `rig_send_morse()` for CW mode sweeps since PTT alone doesn't key the transmitter on the K4.
